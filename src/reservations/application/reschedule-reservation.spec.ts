...
function createContext(options: TestContextOptions = {}) {
  ...
  return {
    ...dependencies,
    original,
    useCase: new RescheduleReservation(dependencies),
  };
}
...
for (const scenario of [
  ...
  {
    name: "a member of another organization",
    user: {
      role: UserRole.Member,
      userId: "user-ana",
  ...
  {
    name: "an administrator of another organization",
    user: {
      role: UserRole.OrganizationAdmin,
      userId: "olivia",
      organizationId: "organization-b",
  ...
  },
  {
    ...
    user: { role: UserRole.PlatformAdmin, userId: "patricia" },
  ...
  test(`rejects rescheduling by ${scenario.name} without changing stored data`, async () => {
    const { useCase, repository, original } = createContext({
      authenticatedUser: scenario.user,
    });
    const before = original.toJSON();
    ...
    assert.deepEqual(
      repository.reservations.map((item) => item.toJSON()),
      [before],
    );
...
for (const startAt of ["2030-05-01T10:00:00Z", "2030-07-30T10:00:00Z"]) {
  test(`accepts the rescheduling booking boundary ${startAt}`, async () => {
    ...
    const endAt = new Date(
      new Date(startAt).getTime() + 15 * 60 * 1000,
    ).toISOString();
    const result = await useCase.execute({
      reservationId: NEW_PERIOD.reservationId,
      startAt,
      endAt,
    });
...
test("rejects rescheduling into an unavailable room", async () => {
  const { useCase, original, repository } = createContext({
    reservableRoomIds: [],
  });
  const before = original.toJSON();
  await assert.rejects(() => useCase.execute(NEW_PERIOD), {
...
test("preserves the original when another organization's reservation conflicts", async () => {
  ...
  const { useCase, repository } = createContext({
    reservations: [original, conflict],
  });
...
test("allows a rescheduled period adjacent to another active booking", async () => {
  ...
  const other = createStoredReservation({
    id: "other",
    startAt: "2030-05-10T11:30:00Z",
    endAt: "2030-05-10T12:30:00Z",
  });
  const { useCase } = createContext({ reservations: [original, other] });
  assert.equal(
    (await useCase.execute(NEW_PERIOD)).endAt,
    "2030-05-10T11:30:00.000Z",
  );
});
